...
class CRM_Smartdebit_Auddis
{
  ...
  static function getSmartdebitCollectionReportForMonth($dateOfCollection) {
    ...
    $emptySql = "TRUNCATE TABLE veda_smartdebit_import";
  ...
  static function saveSmartdebitCollectionReport($collections) {
    $tableName = "veda_smartdebit_import";
  ...
  /**
   * Remove all collection report date from veda_smartdebit_import that is older than one month
   */
  static function removeOldSmartdebitCollectionReports() {
    $date = new DateTime();
    $date->modify('-1 month');
    $dateString = $date->format('Ymd') . '000000';
    $query = "DELETE FROM `veda_smartdebit_import` WHERE receive_date < %1";
