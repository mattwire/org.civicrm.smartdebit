...
/**
 * Class CRM_Core_Payment_SmartDebitIPN
 *
...
class CRM_Core_Payment_SmartDebitIPN extends CRM_Core_Payment_BaseIPN {
  ...
  function recur(&$input, &$ids, &$objects, $first) {
    ...
    switch ( strtolower( $txnType ) ) {
      ...
      case 'recurring_payment':
        ...
        if ( $first ) {
          $recur->payment_instrument_id = CRM_Smartdebit_Base::getDefaultPaymentInstrumentID();
    ...
    if ( !$first ) {
      ...
      $contribution->payment_instrument_id = CRM_Smartdebit_Base::getDefaultPaymentInstrumentID(); // $objects['contribution']->payment_instrument_id;
  ...
  function single( &$input, &$ids, &$objects, $recur = FALSE, $first = FALSE ) {
    ...
    $contribution->payment_instrument_id = CRM_Smartdebit_Base::getDefaultPaymentInstrumentID();
    ...
    if ($first) {
      CRM_Smartdebit_Base::completeDirectDebitSetup( $objects );
    }
  }
  ...
  function main($component = 'contribute') {
    ...
    $paymentProcessorID = CRM_Core_Payment_Smartdebit::getSmartDebitPaymentProcessorID();
